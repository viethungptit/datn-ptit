...
export const updateCurrentUserApi = (payload: { fullName?: string; phone?: string; }) =>
...
export const leaveCompanyForAdminApi = (userId: string) =>
    gatewayApi.put(`/api/user-service/user-profile/employer/for-admin/${userId}/leave`)

export const upsertEmployerApi = (payload: { position?: string; companyId?: string }) =>
    gatewayApi.put(`/api/user-service/user-profile/employer/me`, payload);

export const upsertCandidateApi = (payload: { dob?: string; gender?: string; address?: string; avatarUrl?: string }) =>
    gatewayApi.put(`/api/user-service/user-profile/candidate/me`, payload);

export const upsertEmployerForAdminApi = (userId: string, request: { position?: string; companyId?: string }) =>
    gatewayApi.put(`/api/user-service/user-profile/employer/for-admin/${userId}`, request);

export const upsertCandidateForAdminApi = (userId: string, payload: FormData) =>
    gatewayApi.put(`/api/user-service/user-profile/candidate/for-admin/${userId}`, payload, { headers: { 'Content-Type': 'multipart/form-data' } });
